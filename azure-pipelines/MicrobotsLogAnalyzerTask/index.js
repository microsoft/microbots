...
function runLogAnalyzer(python, inputs) {
  const script = [
    ...
    "print(f'MicrobotsLogAnalyzer: analyzing {log_file_path} with deployment {os.environ[\"OPEN_AI_DEPLOYMENT_NAME\"]}', flush=True)",
    ...
    "bot = LogAnalysisBot(model=f\"azure-openai/{os.environ['OPEN_AI_DEPLOYMENT_NAME']}\", folder_to_mount=codebase_path, token_provider=token_provider)",
  ...
  runCommand(python, ["-c", script, inputs.codebasePath, inputs.logFilePath, inputs.timeoutSeconds], microbotsEnvironment(inputs));
}
